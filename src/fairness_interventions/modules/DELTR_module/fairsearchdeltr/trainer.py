...
~~~~~~~~~~~~~~~
This module holds the detailed mechanics of DELTR_module trainer
"""

import numpy as np
from time import time

from src.modules.DELTR.fairsearchdeltr.models import TrainStep
...
class Trainer(object):
    ...
    def train_nn(self, query_ids, feature_matrix, training_scores, store_losses=False):
        ...
        for t in range(0, self._number_of_iterations):
            ...
            print(str(timestamp) + ' ' + str(omega) + ' ' + str(grad) + ' '+ str(loss_standard) + ' ' + str(loss_exposure) + ' '+ str(np.sum(cost)))
    ...
    def _calculate_cost(self, training_judgments, predictions, query_ids, prot_idx, data_per_query_predicted):
        ...
        implementation of equation 6 in DELTR_module paper
    ...
    def _calculate_gradient(self, training_features, training_judgments, predictions, query_ids,
        ...
        implementation of equation 8 and appendix A in paper DELTR_module
    ...
    def _exposure_diff(self, data, query_ids, which_query, prot_idx):
        ...
        implementation of equation 5 in DELTR_module paper but without the square
        ...
        if isinstance(protected_items_per_query, dict): # more than 2 sensitive groups
            exposer_diff_max = 0
            for group_1 in protected_items_per_query.keys():
                for group_2 in protected_items_per_query.keys():
                    exposer_norm_1 = normalized_exposure(protected_items_per_query[group_1],
                                                       judgments_per_query)
                    exposer_norm_2 = normalized_exposure(protected_items_per_query[group_2],
                                                         judgments_per_query)

                    exposer_diff_temp = abs(exposer_norm_1 - exposer_norm_2)
                    if exposer_diff_temp > exposer_diff_max:
                        exposer_diff_max = exposer_diff_temp
            return exposer_diff_max
        else:
    ...
    def _normalized_topp_prot_deriv_per_group_diff(self, training_features, predictions, query_ids, which_query,
        ...
        implementation of the second factor equation 12 in paper DELTR_module
...
def find_items_per_group_per_query(data, query_ids, which_query, prot_idx):
    ...
    prot_idx_per_query = find_items_per_query(prot_idx,
                                                     query_ids,
                                                     which_query)
    group_set = set(np.array(prot_idx).flatten())
    if len(group_set) > 2:
        dict_group_items = dict()
        for group in group_set:
            dict_group_items[group] = judgments_per_query[np.where(prot_idx_per_query == group)[0], :]
            return judgments_per_query, dict_group_items, []
    else:
...
def normalized_exposure(group_data, all_data):
    ...
    implementation of equation 4 in DELTR_module paper
...
def topp_prot(group_items, all_items):
    ...
    implementation of equation 7 in paper DELTR_module
...
def topp_prot_first_derivative(group_features, all_features, group_predictions, all_predictions):
    ...
    implementation of equation 11 in paper DELTR_module
