...
~~~~~~~~~~~~~~~
This module serves as a wrapper around the utilities we have created for DELTR_module
"""

import numpy as np
import pandas as pd

from src.modules.DELTR.fairsearchdeltr import trainer
...
class Deltr(object):
    ...
    def train(self, training_set: pd.DataFrame):
        """
        Trains a DELTR_module model on a given training set
        ...
        if self._protected_feature_name in names:
            ...
            print(self._protected_feature_name)
    ...
    def rank(self, prediction_set: pd.DataFrame, has_judgment=False):
        """
        Uses the trained DELTR_module model to rank the prediction set
