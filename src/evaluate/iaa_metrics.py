        ...
        for exp_id in exp_unique_id_list:
            for task_id in task_unique_id_list:
                for doc_id in doc_unique_id_list:
                    ...
                    grouped_data = [item for item in users_rates_dataset if
                                    isinstance(item[0], tuple) and item[0][0] == exp_id and item[0][1] == task_id and
                                    item[0][2] == doc_id]
                    # after grouping the doc_id will be the same, so its a good query_id for computing the iaa
                    transformed_data = [[doc_id, user_id, score] for [(exp_id, task_id, doc_id), user_id, score] in
                                        grouped_data]
                    transformed_data = np.array(transformed_data)
                    print("annotate per task", transformed_data)
                    compute_kappas(transformed_data, krippendorffs, cohens, weighted_cohens, filter_attribute,
                                   "annotate")
...
def write_to_file(new_data, filename):
    ...
    modified_dict = {key: value.replace("'", '\"') if isinstance(value, str) else value for key, value in
                     new_data.items()}
...
if __name__ == '__main__':
    ...
    connect(configs_shortlist["data_reader_class"]["name"],
            host=f'mongodb://{"mongo-container"}:{27017}/{configs_shortlist["data_reader_class"]["name"]}', port=27017)
