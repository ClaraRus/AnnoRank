...
//       //Use jQuery AJAX to send data to the Flask app
   ...
   function nextButtonAnnotateCompare() {
      ...
      var checkbox_1 = document.getElementsByName(ranking_type_1)[0];
      var checkbox_2 = document.getElementsByName(ranking_type_2)[0];
      ...
      fetch('/api/'+expId+'/get_next_task_annotate_compare/')
      .then(response => response.json())
      .then(response => {
        console.log(response)
        const nextTask = response.next_task;
